...
# 2.load model
model = Model(reconstruct_pose = args.reconstruct_pose)
...
for e in range(resume_epoch, args.nepochs):
	...
	for train_it, samples in enumerate(loader_train):
		...
		#forward
		optimizer.zero_grad()    
		output = model(poses, gt_locations)                                      # output ~ [batch_size, pred_len, 2] or pose ~ (batch_size, pred_len, 75)
		

		if(args.reconstruct_pose):
			loss = mse_loss(output, gt_poses)
		else:
			loss = mse_loss(output, gt_locations)
	...
	for val_it, samples in enumerate(loader_val):
		...
		#forward
		output = model(poses, gt_locations)                                      # output ~ [batch_size, pred_len, 2]
		if(args.reconstruct_pose):
			loss = mse_loss(output, gt_poses)
		else:
			loss = mse_loss(output, gt_locations)

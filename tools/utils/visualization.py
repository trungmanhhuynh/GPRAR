...
def stgcn_visualize(pose,
    ...
    _, T, V, M = pose.shape  # C, T, V, M
    T = len(video) if video is not None else T
    pos_track = [None] * M
    for t in range(T):
    ...
    for t in range(T):
        ...
        for m in range(M):
            ...
            if label_sequence is not None:
                ...
                body_label = ''
            x_nose = int((pose_rec[0, t, 0, m] + 0.5) * W)
            y_nose = int((pose_rec[1, t, 0, m] + 0.5) * H)
            x_neck = int((pose_rec[0, t, 1, m] + 0.5) * W)
            y_neck = int((pose_rec[1, t, 1, m] + 0.5) * H)

...
class TITANReconstructionAnalysis():
    ...
    def generate_image_result(self):
        ...
        for i in range(0, n_sample, 10):
            ...
            # generate images
            in_pose = result[i]['in_pose'].squeeze(0)
            out_pose = result[i]['out_pose'].squeeze(0)
            gt_pose = result[i]['gt_pose'].squeeze(0)
            video_name = result[i]['video_name'][0]
            image_name = result[i]['image_name'][0]
            video_name = os.path.join(self.args.image_dir, video_name)

            images = gen_reconstructed_pose_on_image(in_pose, out_pose, gt_pose, edge=G.edge,
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--image_dir', type=str, default="results/reconstruction/jaad/image",
                            help='original images dir')
        parser.add_argument('--res_image_dir', type=str, default="results/reconstruction/jaad/image",
...
if __name__ == '__main__':
    ...
    r = TITANReconstructionAnalysis(sys.argv[1:])
    r.start()
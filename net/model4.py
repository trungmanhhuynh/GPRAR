"""
Model 33: Traj-STGCNN. 

This model is different from model 2/3 in the way that:
...
class Model(nn.Module):
    ...
    def __init__(self, pose_features = 3, output_feats = 2, obs_len = 10, pred_len = 10, edge_importance_weighting = False,
                reconstruct_pose = False           # turn on to train the pose reconstructor

                ):
        super().__init__()

        # init variables 
        self.reconstruct_pose = reconstruct_pose
        ...
        self.st_gcn_networks = nn.ModuleList((
            ...
            st_gcn(32, 16,  (temporal_kernel_size, spatial_kernel_size), 1),
            st_gcn(16, 8,  (temporal_kernel_size, spatial_kernel_size), 1),
            st_gcn(8, 3,  (temporal_kernel_size, spatial_kernel_size), 1),
    ...
    def forward(self, pose_in, traj_in  ):
        ...
        # data normalization & re-shape
        N, C, T, V, M = pose_in.size()                          # pose_in ~ [batch_size, pose_features, obs_len, nodes, instances]   
                                                                # e.g. ~ [128, 3, 10, 25, 1]
                                                                # e.g. N = 128, C = 3, T = 10, V = 25, M = 1

        pose_in = pose_in.permute(0, 4, 3, 1, 2).contiguous()   # (128, 3, 10, 25, 1) --> (128, 1, 25, 3, 10)
        pose_in = pose_in.view(N * M, V * C, T)                 # (128, 1, 25, 3, 10) --> (128, 75, 10)
        pose_in = self.data_bn(pose_in)                         
        pose_in = pose_in.view(N, M, V, C, T)                   # (128, 75, 10) --> (128, 1, 25, 3, 10)
        pose_in = pose_in.permute(0, 1, 3, 4, 2).contiguous()   # (128, 1, 25, 3, 10) --> ( 128, 1, 3, 10, 25) 
        pose_in = pose_in.view(N * M, C, T, V)                  # (128, 1, 3, 10, 25) --> (128, 3, 10, 25) 
                                                                # ~ (batch_size, in_channel, obs_len, nodes)
        # re-construct missing keypoints.
        for gcn, importance in zip(self.st_gcn_networks, self.edge_importance):
            pose_in, _ = gcn(pose_in, self.A * importance)      # e.g. pose_in shape ~ (128 , 10 , 10, 25 ) 
                                                                #  ~ (batch_size, out_channels, pred_len, nodes)


        if(self.reconstruct_pose):

            # return reconstruct pose 
            output = pose_in.unsqueeze(4)

        else:

            pose_in = pose_in.permute(0, 1, 3, 2).contiguous()      # (batch_size, out_channels, obs_len, nodes)
            pose_in = pose_in.view(N, V*C, T).contiguous()        # (batch_size, out_channels *nodes, obs_len) 
            pose_in = pose_in.unsqueeze(3)                          # (batch_size, out_channels *nodes, obs_len, 1) 


            # prediction
            if (traj_in is None):
                traj_in = pose_in[:,24:26, :, :]                   #  epose_intract keypoint 8th for locations.
                                                                   #  (batch_size, 3, obs_len (10), 1)  
            else: 
                traj_in = traj_in.permute(0, 2, 1).contiguous()
                traj_in = traj_in.unsqueeze(3)                      # x ~ (batch_size, in_channels (2), obs_len (10), 1)
            ...
            y = y.squeeze(3)                            # pose_in ~ (batch_size, in_channels (2), obs_len (10), 1)

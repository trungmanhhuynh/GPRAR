...
This model is different from model 2 in the way that:
 - pedestrian's locations are extracted from learned pose features from GCN, instead
 from using noisy pose detection.
...
class Model(nn.Module):
    ...
    def __init__(self, pose_features = 3, output_feats = 2, obs_len = 10, pred_len = 10, edge_importance_weighting = False):
        ...
        self.resconstruct = conv2d(in_channels = 256*25, out_channels = 75, kernel_size = 1, stride = 1,  padding = 0)
    ...
    def forward(self, traj_in, x):
        ...
        x = x.permute(0, 1, 3, 2).contiguous()      # (batch_size, out_channels, obs_len, nodes)
        x = x.view(N, V*256, T).contiguous()        # (batch_size, out_channels *nodes, obs_len) 
        x = x.unsqueeze(3)                          # (batch_size, out_channels *nodes, obs_len, 1) 

        x  = self.resconstruct(x)                   # (batch_size, 75, obs_len (10), 1)  

        # Prediction 
        traj_in = x[:,24:27, :, :].clone()          #  extract keypoint 8th for locations.
                                                    #  (batch_size, 3, obs_len (10), 1)  


        traj_y = self.encoder_location(traj_in)
        pose_y = self.encoder_pose(x)

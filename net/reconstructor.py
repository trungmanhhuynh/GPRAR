...
class Reconstructor(nn.Module):
    ...
    def __init__(self, batch_size, in_channels, out_channels, obs_len, pred_len, num_keypoints, edge_importance_weighting):
        super().__init__()
        ...
        # load graph
        self.graph = Graph()
        A = torch.tensor(self.graph.A, dtype=torch.float32, requires_grad=False)    # A ~ [1,25,25] with strategy = 'uniform' and layout = 'pose_25'
    ...
    def forward(self, pose_in):
        ...
        output = output.view(batch_size, self.obs_len, self.num_keypoints * self.out_channels)

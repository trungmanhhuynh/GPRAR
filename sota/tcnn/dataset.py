...
class TrajectoryDataset(Dataset):
    ...
    def __init__(self, data_file, obs_len=10, pred_len=10,
        ...
        if(occluded_rate > 0):
            if(occluded_type == "pose"): self.create_occluded_pose_data(occluded_rate)           # for model using pose: TCNN_POSE
            elif(occluded_type == "locations"): self.create_occluded_location_data(occluded_rate)    # for the model use location: TCNN, LSTM
    ...
    def __getitem__(self, index):
        ...
        if(self.reshape_pose):
            ...
            sample['poses_gt'] = self.poses[index, :, -self.pred_len:, :, :]
            sample['imputed_poses_gt'] = self.imputed_poses[index, :,  -self.pred_len:, :, :]

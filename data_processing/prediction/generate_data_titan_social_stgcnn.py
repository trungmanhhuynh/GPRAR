...
def generate_samples(video_data, video_name, args):
    ...
    for pid, pName in enumerate(id_list):
        ...
        for pose_list, gtloc_list, iname_list, fid_list, pid_list in zip(
                chunks(long_poses, traj_len=args.traj_len, slide=args.traj_len),
                chunks(long_gtloc, traj_len=args.traj_len, slide=args.traj_len),
                chunks(long_inames, traj_len=args.traj_len, slide=args.traj_len),
                chunks(long_fid, traj_len=args.traj_len, slide=args.traj_len),
                chunks(long_pid, traj_len=args.traj_len, slide=args.traj_len)):

            # skip if trajectory is short
            if len(pose_list) < args.traj_len:
                continue
            ...
            # normalize poses
            pose_arr[:, 0::3] = pose_arr[:, 0::3] / float(args.width)  # normalize x by frame width
            pose_arr[:, 1::3] = pose_arr[:, 1::3] / float(args.height)  # normalize y by frame height
            ...
            location = np.zeros((args.traj_len, 2))
            ...
            if args.obs_type == 'gt':
                ...
                location[:, 0] = location[:, 0] / float(args.width)  # normalize x by frame width
                location[:, 1] = location[:, 1] / float(args.height) # normalize y by frame height
...
def generate_data(pose_path, mode, debug=False):
    ...
    for video_name in video_list:
        ...
        for frame_name in frame_list:
            fid = int(frame_name[:6])

            # read data
            with open(os.path.join(args.pose_path, video_name, "{:06d}_keypoints.json".format(fid)), "r") as f:
                pose_data = json.load(f)
            if not os.path.exists(
                    os.path.join(args.location_path, video_name, "{:06d}_locations.json".format(fid))):
                continue
            with open(os.path.join(args.location_path, video_name, "{:06d}_locations.json".format(fid)),
                      "r") as f:
                location_data = json.load(f)

            image_name = "{:06d}.png".format(fid)
        ...
        location_seq, fid_seq, pid_seq = generate_samples(video_data, video_name, args)
        ...
        # write to text file
        out_dir = os.path.join(args.out_folder, 'titan_{}'.format(args.obs_type), mode)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Generate train/val data TITAN for social-stgcnn')
    parser.add_argument(
    ...
    parser.add_argument(
        '--out_folder', default='/home/manhh/github/Social-STGCNN/datasets')
    parser.add_argument(
        '--traj_len', type=int, default=20, help='trajectory len')
    parser.add_argument(
        '--obs_type', type=str, default='raw', help='noisy, impute, or gt')
    parser.add_argument(
    ...
    print("Generate train data TITAN for social-stgcnn")
    generate_data(args.pose_path, "train", args.debug)
    print("Generate val data TITAN for social-stgcnn")

...
def generate_data_flow(args):
    ...
    for video_name in video_list:
        ...
        image_files = os.listdir(os.path.join(args.data_dir, video_name, "images"))
        image_files.sort()
        for i in range(0, len(image_files) - 1):

            img_cur = read_gen(os.path.join(args.data_dir, video_name, "images", image_files[i]))
            img_next = read_gen(os.path.join(args.data_dir, video_name, "images", image_files[i + 1]))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='generate location data in each frame JAAD')
    parser.add_argument(
        '--data_dir', default='/home/manhh/github/datasets/titan/dataset/images_anonymized')
    parser.add_argument(
        '--output_dir', default='/home/manhh/github/datasets/processed_data/features/titan/flow')
    parser.add_argument(

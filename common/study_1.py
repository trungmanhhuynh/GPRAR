...
def calc_ade_per_keypoint(args, observed_pose, pred_traj, gt_locations, video_names, image_names):
    ...
    ax.plot(num_keypoints_per_sample[sort_index], ade_per_sample[sort_index], color='b', linewidth=2.0, label='ade')
    ax.plot(num_keypoints_per_sample[sort_index], ndimage.gaussian_filter1d(ade_per_sample[sort_index], 10), color='r', linewidth=2.0, label='ade')

    plt.xlabel('number of visible keypoints')
    plt.ylabel('ade (pixels)')
    ax.set_ylim(0, 100)
    fig.savefig("study_1_g2.png")
    plt.close()

    print("plotted a graph: study_1_g2.png")

    return num_keypoints_per_sample, ade_per_sample
...
if __name__ == '__main__':
    ...
    #
    print("processing..")
    num_keypoints_per_sample, ade_per_sample = calc_ade_per_keypoint(args, observed_pose, pred_traj, gt_locations, video_names, image_names)

...
TRAJ_FILE = "save/trajs.json"
TEST_DATA = "train_val_data/JAAD/mini_size/val_data.joblib"
...
def plot_trajectory(locations, ax, color = 'r', image_height = 1080):

    ax.plot(locations[:,0], locations[:,1], 'r',  label='observed')
...
def visualize_trajs(image_width = 1920, image_height= 1080, obs_len = 10, pred_len = 10):
    ...
    for s_id in range(len(pred_traj)):
        ...
        ax.plot(locations[s_id,:obs_len,0], locations[s_id,:obs_len,1], color = 'b',  linewidth=2.0, label='observed')
        ax.plot(locations[s_id,-pred_len:,0], locations[s_id,-pred_len:,1], 'r',  linewidth=4.0, label='ground-truth')
